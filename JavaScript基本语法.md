...
一般情况下不同类型会将自身转化为数字类型然后进行比较，有以下几点需要注意

- `object`：会调用`toPrimitive`转换为数字或字符串类型，如果是字符串再进一步转换成数字进行比较
- `null`：会转换为`0`再进行比较
- `undefined`：会转换为`NaN`再进行比较
- `NaN`：总是会返回`false`
...
- `object`：会调用`toPrimitive`转换为数字或字符串类型，如果是字符串再进一步转换成数字进行判等
